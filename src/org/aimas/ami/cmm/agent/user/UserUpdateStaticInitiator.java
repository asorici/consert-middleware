...
public class UserUpdateStaticInitiator extends SimpleAchieveREInitiator {
    private static final long serialVersionUID = 9016768069634061958L;

    private CtxUser ctxUser;
    private UpdateEntityDescriptions entitiesUpdateContent;
    
	public UserUpdateStaticInitiator(CtxUser ctxUser, UpdateEntityDescriptions entitiesUpdateContent) {
	    super(ctxUser, null);
	    
	    this.ctxUser = ctxUser;
	    this.entitiesUpdateContent = entitiesUpdateContent;
    }
	...
	protected ACLMessage prepareRequest(ACLMessage msg) {
		...
		String conversationId = ctxUser.getName() + "-UpdateEntityDescriptions-" + System.currentTimeMillis();
		updateRequest.setConversationId(conversationId);
		updateRequest.addReceiver(ctxUser.getCoordinatorAgent());
		
		try {
			Action updateAction = new Action(ctxUser.getCoordinatorAgent(), entitiesUpdateContent);
