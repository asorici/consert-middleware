...
public class DomainProfiledBroadcastBehaviour extends SequentialBehaviour {
    ...
    private UpdateProfiledAssertion profiledDomainBroadcast;
    private AID receivedFromAgent;
    
	public DomainProfiledBroadcastBehaviour(CtxCoord coordAgent, UpdateProfiledAssertion profiledDomainBroadcast, AID receivedFromAgent) {
		this.ctxCoordinator = coordAgent;
		this.profiledDomainBroadcast = profiledDomainBroadcast;
	...
	private class GetBroadcastBaseInitiator extends SimpleAchieveREInitiator {
		...
		protected ACLMessage prepareRequest(ACLMessage msg) {
			...
			requestContent.setBroadcastLowerBound(profiledDomainBroadcast.getDomain_lower_bound());
			requestContent.setBroadcastUpperBound(profiledDomainBroadcast.getDomain_upper_bound());
		...
		protected Vector<ACLMessage> prepareRequests(ACLMessage msg) {
			...
			for (int i = 0; i < broadcastBase.getBroadcastBaseItem().size(); i++) {
				...
				if (baseItem.getCoordinator().equals(ctxCoordinator)) {
					...
					UpdateProfiledAssertion profiledAssertionUpdate = new DefaultUpdateProfiledAssertion();
					profiledAssertionUpdate.setAssertion(profiledDomainBroadcast.getAssertion());
					profiledAssertionUpdate.setAssertionContent(profiledDomainBroadcast.getAssertionContent());
					
					ctxCoordinator.getContextUpdateManager().updateProfiledAssertion(receivedFromAgent, profiledAssertionUpdate);
				}
				else {
					...
					try {
						Action forwardAction = new Action(baseItem.getCoordinator(), profiledDomainBroadcast);
